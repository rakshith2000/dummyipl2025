...
sqclr = {
    ...
    'IND': {'c1': 'hsl(310 89% 52%)', 'c2': 'hsl(208 100% 31%)'},   # Pink to Blue
    'BAN': {'c1': 'hsl(225, 31%, 15%)', 'c2': 'hsl(44, 60%, 65%)'},   # Navy to Gold
    'PAK': {'c1': 'hsl(310 89% 52%)', 'c2': 'hsl(208 100% 31%)'},   # Pink to Blue
    'SL': {'c1': 'hsl(225, 31%, 15%)', 'c2': 'hsl(44, 60%, 65%)'},   # Navy to Gold
...
def render_live_URL(tA, tB, mn, dt):
    ...
    if mn.lower() not in ['final', 'f']:
        if int(mn) == 1:
            URL = "https://cmc2.sportskeeda.com/live-cricket-score/sri-lanka-vs-bangladesh-super-four-match-1-(b1-v-b2)-20-september-2025/ajax"
        elif int(mn) == 2:
            URL = "https://cmc2.sportskeeda.com/live-cricket-score/india-vs-pakistan-super-four-match-2-(a1-v-a2)-21-september-2025/ajax"
        elif int(mn) == 3:
            URL = "https://cmc2.sportskeeda.com/live-cricket-score/pakistan-vs-sri-lanka-super-four-match-3-(a2-v-b1)-23-september-2025/ajax"
        elif int(mn) == 4:
        ...
        if inn["fall_of_wickets"] is not None:
            ...
            if inn["fall_of_wickets"] != "":
                for bt in inn["fall_of_wickets"].split('),'):
                    n = bt.split(' (')[1].split(',')[0]
                    score = bt.split(' (')[0]
                    over = bt.split(' (')[1].split(', ')[1].strip('()')
                    fow.append({"name": n, "score": score, "over": over})
